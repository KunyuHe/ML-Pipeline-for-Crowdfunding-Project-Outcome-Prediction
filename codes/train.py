...
from trainviz import (plot_predicted_scores, plot_precision_recall, plot_auc_roc,
...
def load_features(batch):
    """
    Load pre-processed feature matrices.

    """
    Xs = np.load(INPUT_DIR + 'X{}.npz'.format(batch))
    ys = np.load(INPUT_DIR + 'y{}.npz'.format(batch))
...
def find_best_threshold(model_index, metric_index, train_data,
    ...
    if plot:
        ...
        plot_predicted_scores(default_probs, BATCH, "({} -- {})".format(model_name, metric_name))
...
def evaluate_best_model(model_index, metric_index, best_threshold, best_grid, data,
    ...
    if plot:
        ...
        plot_precision_recall(y_test, predicted_prob, baseline, BATCH, "({} -- {})".\
                              format(model_name, metric_name))

        plot_auc_roc(clf, data, BATCH, "({} -- {})".format(model_name, metric_name))

        if hasattr(clf, "feature_importances_"):
            importances = clf.feature_importances_
            with open(INPUT_DIR + 'col_names.pickle', 'rb') as handle:
                col_names = pickle.load(handle)
            plot_feature_importances(importances, col_names, BATCH, title="({} -- {})".\
                                     format(model_name, metric_name))
...
if __name__ == "__main__":
    ...
    print(("You can either choose to train with a specific configuration (input 1)"
           " on data from the first time window, or all the models by metrics we"
           " have (input 2) on all the time windows."))
    flag = int(input("Please input your choice:\n"))

    if flag == 1:
        X_train, X_test, y_train, y_test = load_features(0)
